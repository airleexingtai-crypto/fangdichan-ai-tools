...
export const metadata = generatePageMeta({
  ...
  description: "Compare AI tools for real estate side by side. See pricing, features, and pros & cons in one view.",
  path: "/en/compare",
});
...
export default async function ComparePage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <BreadcrumbNav items={[{ label: "Compare", href: "/en/compare" }]} />
      ...
      {comparisons && comparisons.length > 0 ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {comparisons.map((c: any) => (
            <Link key={c.slug} href={`/en/compare/${c.slug}`} className="no-style">
        ...
        <div className="text-center py-16 text-muted-foreground">
          <p>No comparisons yet. Start by browsing tools and selecting "Compare".</p>
        </div>
      )}
    </div>
  );
