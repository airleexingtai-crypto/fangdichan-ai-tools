...
export const metadata = generatePageMeta({
  ...
  description: "Key statistics and data about AI adoption in real estate. Market size, adoption rates, and trends.",
  path: "/en/stats",
});
...
export default async function StatsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
      <BreadcrumbNav items={[{ label: "Statistics", href: "/en/stats" }]} />
      <h1 className="text-3xl font-bold mt-4 mb-6">AI in Real Estate — Statistics & Data</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {stats?.map((s: any) => (
          <Link key={s.slug} href={`/en/stats/${s.slug}`} className="no-style">
